export class GraphLayout {
    ...
    mapEdgesToNodes() {
        ...
        this.edges.forEach(edge => {
            ...
            if (sourceNode) {
                ...
                parentNodeIdForNode[edge.target] = parentNodeIdForNode[edge.target] || new Set();
    ...
    sortNodesIntoBaskets(sourceTypesForNode) {
        ...
        this.nodes.forEach(node => {
            ...
            if (type === 'DataSourceVariable' && id.includes('Data')) {
                targetBasket = sourceTypesForNode[node.id] ? 'dataFieldWithTarget' : 'dataFieldNoTarget';
        ...
        const calculateDepth = (node, basketTypeName) => {
            ...
            parentIds.forEach(parentId => {
                ...
                if (parent && parent.data.__typename === basketTypeName) {
                    let depth = adjustedDepths[parent.id] !== undefined ? adjustedDepths[parent.id] + 1 : 1;
    ...
    generateLayout() {
        ...
        Object.keys(baskets).forEach(basketKey => {
            let maxDepth = this.applyWaterfallEffect(baskets[basketKey], startX, startY, adjustedDepths);
